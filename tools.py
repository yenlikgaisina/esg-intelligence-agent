...
def save_report(markdown: str, metrics_json: str = "{}") -> str:
    """Save today's briefing as markdown + PDF, append summary to history."""
...
# ── TOOL 7: OpenAI cross-check ────────────────────────────────────────────

def openai_analysis(prompt: str, context: str = "") -> str:
    """Send a prompt to GPT-4o for a second-opinion cross-check."""
...
TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "description": "Send a prompt to GPT-4o for a second-opinion cross-check. Gracefully skipped if OPENAI_API_KEY is not set.",
        "input_schema": {
            "type": "object",
            "properties": {
                "prompt": {"type": "string", "description": "The question or analysis request for GPT-4o"},
                "context": {"type": "string", "description": "Optional system context (e.g. regulation excerpt or LCA output to review)"},
            },
            "required": ["prompt"],
        },
    },
